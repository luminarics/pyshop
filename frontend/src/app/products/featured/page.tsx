...
import { Star, ShoppingBag, RefreshCw, ArrowRight } from "lucide-react";
...
function FeaturedProductsContent() {
  ...
  // Filter for featured products (you can adjust this logic based on your needs)
  // For now, we'll show the top-rated or newest products
  ...
  const addToCartMutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      const product = featuredProducts.find((p) => p.id === variables.productId);
    ...
    <ResponsiveContainer size="2xl" className="py-8">
      ...
      {!isLoading && total > 0 && (
        <div className="mb-4 text-sm text-muted-foreground">
          Showing {((page - 1) * pageSize) + 1} to {Math.min(page * pageSize, total)} of {total} featured products
        </div>
      )}
